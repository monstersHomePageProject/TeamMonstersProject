...
public final class ConnectionPool {
  ...
  static final String jdbcclass = "oracle.jdbc.OracleDriver"; //오라클 커넥션Pool  
 // Storage for the allocated connections.
    ...
    // The number of connection that have been created. (현재 커넥션 수)
    private int numCons = 0;

    // Whether to block until a connection is free when maxCons are in use.
    // 대여중인 커넥션이 꽉 차있을 때 대기시키는 필드 (참이면 대기, 거짓이면 대기X) 
     ...
     * @see	  java.sql.DriverManager
     **/
  //업무용으로 중요,  전체를 synchronized를 걸면 다 수행될 때까지 기다려야되기 때문에 굉장히 느려짐.
...
throws SQLException {
  ...
  if (free.isEmpty()) {
    ...
    // None left, do we create more?
	 /*
	  * maxCons <= 0 : maxCons을 설정하지 않음(무한대)
	  * numCons < maxCons : 커넥션이 다 차지 않았을 때
	  */
  ...
  /*
   * used는 반납을 확실히 받기 위해서 synchronized를 사용.
   */
  synchronized (used) {
...
throws SQLException {
  ...
  try {
	...
	free.addElement(getNewConnection()); //생성된 커넥션을 추가
    }
...
System.out.println("About to connect to " + url);
 try {
	  con = DriverManager.getConnection(url, user, password); //커넥션 생성 (msql은 url만 들어가면 되지만 오라클은 user, pwd도 같이 넣어야 함.)
}
 catch (Exception e) {
    e.printStackTrace();
 }
